...
def draw_forces(params, filepath, targetfile, forces):
...
excluded = ['3b3.csv','3b8.csv','10.csv']
trj_files = [x for x in os.listdir(os.path.join(DRIVE,PROJECT)) if choose_trj(x,'.csv',excluded)]
...
for file in trj_files:
    ...
    force_dirs, force_mags = forces_from_file(params,filepath,field_dir=[1,0,0])
...
for file in trj_files:
    ...
    tgfile = file[:-4] + '.pdf'
    draw_forces(params,filepath,os.path.join(DRIVE,PROJECT,tgfile),forces)
